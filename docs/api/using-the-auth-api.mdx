...
- `verifcationCode` which the user will see in their device and we strongly recommend to display on your end. This is only used when `AuthenticationContextType` is set to `App` for `Sim` it will be empty
