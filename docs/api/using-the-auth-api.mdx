...
* Then every two seconds call `/api/auth/poll` to check whether the user has authenticated
...
- `authRequestId` is a unique key for this login session, you will need to send every two seconds to `/api/auth/poll` (see below) until user logs in or request times out (180 seconds).
